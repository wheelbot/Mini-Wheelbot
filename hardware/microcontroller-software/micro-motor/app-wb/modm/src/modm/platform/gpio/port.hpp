...
public:
	static PortType isSet()
	{
		...
		if constexpr (mask(3)) r = (GPIOF->ODR & mask(3)) ^ inverted(3);
		if constexpr (mask(4)) r = (GPIOG->ODR & mask(4)) ^ inverted(4);
	...
	static PortType read()
	{
		...
		if constexpr (mask(3)) r = (GPIOF->IDR & mask(3)) ^ inverted(3);
		if constexpr (mask(4)) r = (GPIOG->IDR & mask(4)) ^ inverted(4);
		if constexpr (getDataOrder() == modm::GpioPort::DataOrder::Reversed)
			 return bitReverse(r) >> StartPinReversed;
		else return            r  >> StartPin;
	}
	...
	static void write(PortType data)
	{
		...
		if constexpr (mask(3)) {
			p ^= inverted(3);
			GPIOF->BSRR = ((~p & mask(3)) << 16) | (p & mask(3));
		}
		if constexpr (mask(4)) {
			p ^= inverted(4);
			GPIOG->BSRR = ((~p & mask(4)) << 16) | (p & mask(4));
		}
	}
};
