...
bool
modm::platform::Adc2::setChannelSequence(std::span<const SequenceChannel> sequence)
{
	if (sequence.size() > 16 || sequence.size() == 0) {
		return false;
	}

	ADC2->SQR1 = sequence.size() - 1;
	...
	for (const auto [i, config] : modm::enumerate(sequence)) {
		...
		if (i < 4) {
			...
			ADC2->SQR1 = (ADC2->SQR1 & ~mask) | (std::to_underlying(channel) << shift);
		} else if (i < 9) {
			const auto shift = (i - 4) * 6;
			const auto mask = 0b111111u << shift;
			ADC2->SQR2 = (ADC2->SQR2 & ~mask) | (std::to_underlying(channel) << shift);
		} else if (i < 14) {
			const auto shift = (i - 9) * 6;
			const auto mask = 0b111111u << shift;
			ADC2->SQR3 = (ADC2->SQR3 & ~mask) | (std::to_underlying(channel) << shift);
		} else {
			const auto shift = (i - 14) * 6;
			const auto mask = 0b111111u << shift;
			ADC2->SQR4 = (ADC2->SQR4 & ~mask) | (std::to_underlying(channel) << shift);
		}
	}
	return true;
}
...
void
modm::platform::Adc2::stopConversion()
{
	ADC2->CR |= ADC_CR_ADSTP | ADC_CR_JADSTP;
	while ((ADC2->CR & (ADC_CR_ADSTP | ADC_CR_JADSTP)) != 0);
...
bool
modm::platform::Adc2::isConversionSequenceFinished()
{
...
void
modm::platform::Adc2::setDmaMode(DmaMode mode)
{
	constexpr uint32_t mask = std::to_underlying(DmaMode::Mask);
	ADC2->CFGR = (ADC2->CFGR & ~mask) | std::to_underlying(mode);
}
