...
bool
modm::platform::Adc4::configureChannel(Channel channel,
...
bool
modm::platform::Adc4::setChannel(Channel channel,
										SampleTime sampleTime)
{
	...
	ADC4->SQR1 = (static_cast<uint8_t>(channel) << ADC_SQR1_SQ1_Pos) & ADC_SQR1_SQ1_Msk;
	return true;
}

bool
modm::platform::Adc4::setChannelSequence(std::span<const SequenceChannel> sequence)
{
	if (sequence.size() > 16 || sequence.size() == 0) {
		return false;
	}

	ADC4->SQR1 = sequence.size() - 1;
	...
	for (const auto [i, config] : modm::enumerate(sequence)) {
		...
		if (i < 4) {
			...
			ADC4->SQR1 = (ADC4->SQR1 & ~mask) | (std::to_underlying(channel) << shift);
		} else if (i < 9) {
			const auto shift = (i - 4) * 6;
			const auto mask = 0b111111u << shift;
			ADC4->SQR2 = (ADC4->SQR2 & ~mask) | (std::to_underlying(channel) << shift);
		} else if (i < 14) {
			const auto shift = (i - 9) * 6;
			const auto mask = 0b111111u << shift;
			ADC4->SQR3 = (ADC4->SQR3 & ~mask) | (std::to_underlying(channel) << shift);
		} else {
			const auto shift = (i - 14) * 6;
			const auto mask = 0b111111u << shift;
			ADC4->SQR4 = (ADC4->SQR4 & ~mask) | (std::to_underlying(channel) << shift);
		}
	}
	return true;
}
...
void
modm::platform::Adc4::startConversion()
{
...
void
modm::platform::Adc4::stopConversion()
{
	ADC4->CR |= ADC_CR_ADSTP | ADC_CR_JADSTP;
	while ((ADC4->CR & (ADC_CR_ADSTP | ADC_CR_JADSTP)) != 0);
...
bool
modm::platform::Adc4::setInjectedConversionChannel(uint8_t index, Channel channel,
														  SampleTime sampleTime)
{
	if (index >= 4) {
		return false;
	...
	ADC4->JSQR = (ADC4->JSQR & ~mask) | (static_cast<uint32_t>(channel) << pos);
	return true;
}

template<class Gpio>
bool
modm::platform::Adc4::setInjectedConversionChannel(uint8_t index,
														  SampleTime sampleTime)
{
...
bool
modm::platform::Adc4::setInjectedConversionSequenceLength(uint8_t length)
{
	if (length < 1 or length > 4) {
		return false;
	}
	ADC4->JSQR = (ADC4->JSQR & ~ADC_JSQR_JL)
		| ((length - 1) << ADC_JSQR_JL_Pos);
	return true;
}

bool
modm::platform::Adc4::isInjectedConversionFinished()
{
	return static_cast<bool>(getInterruptFlags() & InterruptFlag::EndOfInjectedSequenceOfConversions);
}

void
modm::platform::Adc4::setDmaMode(DmaMode mode)
{
	constexpr uint32_t mask = std::to_underlying(DmaMode::Mask);
	ADC4->CFGR = (ADC4->CFGR & ~mask) | std::to_underlying(mode);
}
...
void
modm::platform::Adc4::acknowledgeInterruptFlags(const InterruptFlag_t flags)
{
